...
# Code that tests wether a valid input is given
...
# Code that tests wether a valid input is given (v1.1)
...
# for a number. A function is a chunck of code that does something.
# Functions can be used over and over again.
# To use a function (call it), write out it's name with parenthesis at the end.
...
'''def test_intnum():
    ...
    while not done:
        try:
            num = int(input("Please input a number (integer)"))
            done = True
        except ValueError:
            print("That is not a vaild float number. \n")
    print(f"The number that you entered is {num}.")



# Main routine 
test_intnum() # This calls the function so that we can use it'''

# Code that tests wehther a valid input is given (v.1.3)
...
# Use the function parameters to make my code my pythonic.

#Main routine 
def valid_num(question,low,high):
    error = f"Whoops, that is not a integer between {low}, {high}."
    while True:
        try:
            response = int(input(question))
            # If the response >= 0 and response <= 10:
...
# Multiply and add the result of num_1 and num_2
sum = num_1 * num_2
print(f"The total of {num_1} and {num_2} multiplied is {sum}.\n")
sum = num_1 + num_2
print(f"The total of {num_1} and {num_2} added together is {sum}.\n") 